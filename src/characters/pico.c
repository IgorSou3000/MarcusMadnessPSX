...
void Character_DrawFlipped(Character *this, Gfx_Tex *tex, const CharFrame *cframe)
{
	...
	Stage_DrawTexCol(tex, &src, &dst, stage.camera.bzoom, 128, 128, 128);
}
...
void Char_Pico_Tick(Character *character)
{
	...
	Character_DrawFlipped(character, &this->tex, &char_pico_frame[this->frame]);
}
...
Character *Char_Pico_New(fixed_t x, fixed_t y)
{
	...
	//health bar color
	this->character.health_b = 0xFFB5D652;
