...
void Stage_Load(StageId id, StageDiff difficulty, boolean story)
{
    ...
    if (stage.stage_id == StageId_Mushroom)
        muststart = 4 * 4;
    else
        muststart = 0;
    mustend = muststart + 30;
    //Check movies
...
void Stage_Tick(void)
{
    ...
    switch (stage.state)
    {
        ...
        {       
            ...
            {
                RECT scr_src = {0, 0, 255, 255};
                RECT scr_dst = {0, 0, 320, 240};
                Gfx_BlendTex(&stage.tex_credit, &scr_src, &scr_dst, (stage.song_step >= mustend ? 1 : 0));
            }
